...
data class Mark(
    ...
    companion object {
        fun random():Mark {
            val shape = MarkShape.values().toList().shuffled().first()
            val color = Color.values().filter{it !== Color.Black}.toList().shuffled().first()
            if(shape == MarkShape.Any || color == Color.Any) return Mark(MarkShape.Any, Color.Any)
            return Mark(shape, color)
        }
    }
}
...
data class BoardPart(
    ...
    fun rotate90At(width: Int = 8): BoardPart {
        ...
        return BoardPart(BoardPartId(0),Color.Any, rotMirrors, rotWalls, rotMarks)
    }
}
