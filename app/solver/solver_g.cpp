...
struct Tile {
	...
	int color = NOCOLOR, nextdir[4] = { UP, RIGHT, DOWN, LEFT}, mirror_color = NOCOLOR;
};
...
public:
	...
	inline int Estimate(RobotPos & robots, int idx, pii to, int level) {
		if (idx == -1) {
			int Min = INT_MAX;
			REP(i,5) chmin(Min, Estimate(robots, i, to, level));
			return Min;
		}
	...
	int DFS(RobotPos & robots, int idx, pii to, vector<pair<int, pii>> & history,
		...
		bool hasGoal = false;
		if (idx != -1) { // 目的がAnyじゃない
			hasGoal = robots[idx] == to;
		}
		else { 
			//どのロボットが到達しても良い、可能性があるのは最後に動いたロボット(br)
			hasGoal = robots[br] == to;
			if(depth == 0) {
	...
	pair<bool,vector<pair<int, pii>>> DFSSolve(RobotPos robots, int idx, pii to, int bound, int mxlv, bool res = true) {
		vector<pair<int, pii>> ret;
		int flag = DFS(robots, idx, to, ret, &bound, mxlv, 0,-3,  0, res);
		reverse(ALL(ret));
		return MP(flag != -1, ret);
	}
...
signed main(void) {
	...
	while (1)
	{
		...
		if (sol.first == false)
			continue;
		cout << sol.second.size() << endl;
		for (auto itr : sol.second)
		{
